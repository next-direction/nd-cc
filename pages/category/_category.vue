...
<script>
    ...
    const fetchPages = async (app, params, store) => {
        ...
        const pageResponse = await fetch(
            store.state.baseUrl
            + `/items/page?filter[category][in]=${categories.join(',')}&sort=status,-created_on&fields=id,title,status,created_on,category.id,created_by.last_name,children.id,children.accepted&filter[parent_page][eq]=0`,
            {
                headers,
            },
        );
