...
<script>
    ...
    const fetchPages = async (app, params, store) => {
        ...
        // fetch pages
        const pageResponse = await fetch(store.state.baseUrl + `/items/page?filter[category][in]=${categories.join(',')}&sort=status,-created_on&fields=id,title,status,created_on,category.id,created_by.last_name`, {
            headers,
        });
